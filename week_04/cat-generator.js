...
function generateCatElement(catData) {
  ...
  // yy
  /* event listeners */
  // click the cat and change body color
  catContainer.addEventListener("mouseenter", colorStartChange);
  catContainer.addEventListener("mouseleave", colorStopChange);
  function colorStartChange() {
    catBody.classList.add("cc");
  }
  function colorStopChange() {
    catBody.classList.remove("cc");
  }
...
// yy
const colorList = ["#FF0054", "#FF5400", "#FFBD00", "#07ED94","#390099"];
const bubbleList = ["Touch my body!","Don't click me!","Touch me!","Yuyuan's Cat"];
const faceList = ["Φ ω Φ","< w <","≧ ∀ ≦","눈 ▂ 눈","• ㅂ •","・ ∀ ・"];
...
const catList = [
  {
    ...
    face: faceList[Math.floor(Math.random() * faceList.length)],
    message: bubbleList[Math.floor(Math.random() * bubbleList.length)],
    isSitting: true,
    numberOfPaws: 4,
    weight: 80 //chonky
  },
